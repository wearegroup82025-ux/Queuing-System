...
// Check if counter is selected
if (!isset($_SESSION['counter'])) {
  header("Location: selectCounter.php");
  exit;
}

$counter = $_SESSION['counter']; // Get the selected counter number
...
// Fetch queue/reservation data only for the selected counter
...
// Count totals for the selected counter
$total = $conn->prepare("SELECT COUNT(*) AS total FROM register WHERE counter = ?");
$total->bind_param("s", $counter);
$total->execute();
$totalCount = $total->get_result()->fetch_assoc()['total'];

$countApproved = $conn->query("SELECT COUNT(*) AS total FROM register WHERE counter = '$counter' AND status='Approved'")->fetch_assoc()['total'];
$countPending = $conn->query("SELECT COUNT(*) AS total FROM register WHERE counter = '$counter' AND (status='Pending' OR status='' OR status IS NULL)")->fetch_assoc()['total'];
?>
...
<body>
    ...
    <main class="content">
      ...
      <section id="dashboard" class="section active">
        ...
        <div class="card-container">
          <div class="card">
            <h3>Total Students (Counter 1)</h3>
  ...
  <tbody>
    ...
    if ($result && $result->num_rows > 0) {
      ...
      echo "<tr><td colspan='6' style='text-align:center;'>No records found for Counter 1.</td></tr>";
    }
    ?>
  </tbody>
</table>
      ...
      <section id="reports" class="section">
        <div class="page-header" style="display:flex; justify-content:space-between; align-items:center;">
          <h2>Reports (Counter 1)</h2>
          <button id="refreshReports" onclick="location.reload()">Refresh Reports</button>
        </div>
        <p>Summary of queue records for Counter 1 as of <?php echo date("M d, Y"); ?></p>
    ...
    new Chart(ctx, {
      ...
      options: {
        ...
        plugins: {
          ...
          title: { display: true, text: 'Queue Summary (Counter 1)' }
        },
